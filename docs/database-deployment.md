...
Routine hosting builds run `npm run build` and never attempt schema changes. Apply migrations first with `npm run deploy:release`, or use `npm run build:release` only in a controlled builder that has the owner credential. This keeps owner access out of Hostinger's application runtime while allowing ordinary redeployments to compile with the restricted credential.
