...
4. The Hostinger `npm run build` command generates Prisma Client, runs the guarded production migration deployment, and only then starts the Next.js build. Configure `MIGRATION_DATABASE_URL` as a build-only Neon owner connection and prefer a restricted `DATABASE_URL` for runtime. The runner recognizes standard unpooled owner aliases. For Hostinger installations that expose only `DATABASE_URL`, it uses that URL only after an ownership probe proves the role owns the migration ledger and every public table and can create schema objects. A failed ownership check stops the build before prerendering. `npm start` performs a read-only identity/schema preflight, while the separate `vercel-build` command remains migration-free.
