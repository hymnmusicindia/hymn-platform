...
4. The Hostinger `npm run build` command generates Prisma Client, runs the guarded production migration deployment, and only then starts the Next.js build. Configure `MIGRATION_DATABASE_URL` as a build-only Neon owner connection; keep `DATABASE_URL` restricted for runtime. The runner also recognizes standard unpooled owner aliases and never falls back to the runtime role. A failed migration stops the build before prerendering. `npm start` performs a read-only identity/schema preflight, while the separate `vercel-build` command remains migration-free.
