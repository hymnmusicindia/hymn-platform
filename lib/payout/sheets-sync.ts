...
export const syncPayoutRequestToSheet = (row: any) => append(TABS.payout, [row.id, row.userId, String(row.amount), String(row.serviceFee), String(row.netAmount), row.currency, row.method, row.status, row.requestedAt]);
