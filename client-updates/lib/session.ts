...
const adminSecret = process.env.ADMIN_JWT_SECRET || `${secret}:admin`;
...
export async function createSession(payload: SessionPayload) {
  ...
  const token = jwt.sign({ ...payload, sid: sessionId }, secret, { expiresIn: "7d" });
...
export async function createAdminSession() {
  const token = jwt.sign({ username: "admin", role: "admin" } satisfies AdminSessionPayload, adminSecret, { expiresIn: "1d" });
  const store = await getCookieStore();
...
export async function getAdminSession() {
  ...
  if (!value) return null;
  try {
    const decoded = jwt.verify(value, adminSecret);
    if (typeof decoded === "string") return null;
