        ...
        <div className="absolute top-full z-30 mt-2 w-full rounded-[1.6rem] border p-3 shadow-2xl" style={{ borderColor: "var(--border)", background: "var(--bg-elevated)" }}>
          ...
          {!query.trim() && recent.length > 0 ? (
            <div className="grid gap-2">
              <p className="px-2 text-xs uppercase tracking-[0.18em]" style={{ color: "var(--text-soft)" }}>Recently used</p>
              {recent.map((profile) => (
                <button key={`recent-${profile.id}`} type="button" onClick={() => selectSaved(profile)} className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left transition-transform duration-200 hover:scale-[1.01]" style={{ background: "var(--card)", color: "var(--text)" }}>
                  <ArtistAvatar name={profile.name} imageUrl={profile.imageUrl} />
                  <div className="min-w-0 flex-1">
                    <p className="truncate font-medium">{profile.name}</p>
                    <p className="truncate text-xs" style={{ color: "var(--text-soft)" }}>Saved profile</p>
                  </div>
                  <span className="text-xs" style={{ color: "var(--text-soft)" }}>Recent</span>
                </button>
              ))}
            </div>
          ) : null}

          {query.trim() && savedMatches.length > 0 ? (
            <div className="grid gap-2">
              <p className="px-2 text-xs uppercase tracking-[0.18em]" style={{ color: "var(--text-soft)" }}>HYMN artist profiles</p>
              {savedMatches.map((profile) => (
                <button key={`saved-${profile.id}`} type="button" onClick={() => selectSaved(profile)} className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left transition-transform duration-200 hover:scale-[1.01]" style={{ background: "var(--card)", color: "var(--text)" }}>
                  <ArtistAvatar name={profile.name} imageUrl={profile.imageUrl} />
                  <div className="min-w-0 flex-1">
                    <p className="truncate font-medium">{profile.name}</p>
                    <p className="truncate text-xs" style={{ color: "var(--text-soft)" }}>{profile.followers ? `${profile.followers.toLocaleString("en-IN")} followers` : "Saved profile"}</p>
                  </div>
                  <span className="text-xs" style={{ color: "var(--text-soft)" }}>Saved</span>
                </button>
              ))}
            </div>
          ) : null}

          {query.trim() && savedMatches.length === 0 && !loading ? (
            <button type="button" onClick={openCreateModal} className="mt-2 flex w-full items-center justify-between rounded-xl border px-3 py-3 text-left transition-transform duration-200 hover:scale-[1.01]" style={{ borderColor: "var(--border)", background: "var(--card)", color: "var(--text)" }}>
              <span>{`+ Add "${query.trim()}"`}</span>
              <span className="text-xs" style={{ color: "var(--text-soft)" }}>New</span>
            </button>
          ) : null}
        </div>
      ) : null}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4 py-6">
          <div className="w-full max-w-2xl rounded-[1.8rem] border p-6 shadow-2xl" style={{ borderColor: "var(--border)", background: "var(--bg-elevated)" }}>
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-sm uppercase tracking-[0.22em]" style={{ color: "var(--text-soft)" }}>Add artist</p>
                <h3 className="mt-3 text-2xl font-semibold" style={{ color: "var(--text)" }}>{`Create ${label.toLowerCase()}`}</h3>
                <p className="mt-2 text-sm" style={{ color: "var(--text-muted)" }}>Search Spotify, or keep the entry manual if there is no live profile to resolve.</p>
              </div>
            ...
            <div className="mt-5 grid gap-4">
              ...
              <div>
                <label className="mb-2 block text-sm font-medium" style={{ color: "var(--text-muted)" }}>Spotify link fallback</label>
                <input className="field" value={manualSpotifyUrl} onChange={(event) => setManualSpotifyUrl(event.target.value)} placeholder="https://open.spotify.com/artist/..." />
              </div>
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-end">
              ...
              <button type="button" className="btn-primary pressable" onClick={() => void saveArtist()} disabled={saving}>{saving ? "Saving..." : `Save ${label}`}</button>
            </div>
          </div>
        </div>
      ) : null}
