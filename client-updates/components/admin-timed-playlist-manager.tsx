...
function SkeletonRow() {
  return <div className="h-28 animate-pulse rounded-[1.25rem] border border-white/10 bg-white/[0.04]" />;
}
  ...
  return (
    <article className="rounded-[1.25rem] border border-white/10 bg-white/[0.03] p-4">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="grid gap-3 rounded-[1.1rem] border border-white/10 bg-black/20 p-4 text-sm text-white/70 lg:min-w-[180px]">
          <div>
      ...
      {isExtending ? (
        <div className="mt-4 grid gap-3 rounded-[1.1rem] border border-white/10 bg-black/30 p-4 sm:grid-cols-[1fr,auto] sm:items-end">
          ...
          <div className="flex gap-2">
            <button type="button" onClick={() => onExtendValueChange(addMinutesToDatetimeLocal(extendValue, 15))} className="rounded-full border border-white/10 bg-white/[0.04] px-4 py-3 text-sm font-semibold text-white/80 transition hover:bg-white/[0.08]">
              +15m
            </button>
            <button type="button" onClick={() => onExtendValueChange(addMinutesToDatetimeLocal(extendValue, 60))} className="rounded-full border border-white/10 bg-white/[0.04] px-4 py-3 text-sm font-semibold text-white/80 transition hover:bg-white/[0.08]">
              +60m
            </button>
            ...
            <button type="button" onClick={onCancelExtend} className="rounded-full border border-white/10 bg-white/[0.04] px-4 py-3 text-sm font-semibold text-white/70 transition hover:bg-white/[0.08]">
              Cancel
            </button>
          </div>
        </div>
...
export function AdminTimedPlaylistManager() {
  ...
  return (
    <section className="rounded-[2rem] border border-white/10 bg-[#0f0f0f] p-4 text-white shadow-[0_24px_64px_rgba(0,0,0,0.35)] sm:p-6">
      <div className="mb-6 flex flex-col gap-3 border-b border-white/10 pb-6 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <span className="inline-flex items-center rounded-full border border-white/10 bg-white/[0.04] px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.28em] text-white/60">Timed Playlist Manager</span>
        ...
        <div className="rounded-[1.25rem] border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-white/60">
      ...
      <div className="mb-6 rounded-[1.25rem] border border-white/10 bg-white/[0.03] p-4">
      ...
      <div className="grid gap-6 lg:grid-cols-[240px,minmax(0,1fr)]">
        ...
        <main className="rounded-[1.5rem] border border-white/10 bg-[#151515] p-4 sm:p-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-3 text-xs text-white/45">
              ...
              <button type="button" onClick={() => void refreshDashboard(true)} className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-4 py-2 text-sm font-semibold text-white transition hover:bg-white/[0.08]">
                      ...
                      <div className="rounded-[1.25rem] border border-white/10 bg-white/[0.03] p-5">
                        ...
                        <div className="mt-4 grid gap-3">
                          {liveActiveTracks.slice(0, 3).map((track) => (
                            <div key={track.id} className="flex items-center justify-between gap-3 rounded-2xl border border-white/10 bg-black/20 px-4 py-3">
                              <div className="min-w-0">
                        ...
                        <div className="relative">
                          ...
                          {form.spotifyUrl.trim() ? (
                            <button
                              ...
                              className="absolute right-3 top-1/2 inline-flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-full border border-white/10 bg-white/[0.04] text-white/55 transition hover:bg-white/[0.08] hover:text-white"
                          ...
                          {trackSearchResults.length ? (
                            <div className="absolute left-0 right-0 top-full z-20 mt-2 overflow-hidden rounded-[1.5rem] border border-white/10 bg-[#111111] shadow-[0_24px_64px_rgba(0,0,0,0.45)]">
                                ...
                                {trackSearchResults.map((track) => (
                                  ...
                                  >
                                    {track.imageUrl ? (
                                      ...
                                      <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-white/10 bg-white/[0.04] text-white/45">
                ...
                {activeView === "active-tracks" ? (
                  <div className="grid gap-4">
                    {liveActiveTracks.length ? liveActiveTracks.map((track) => (
                      ...
                      />
                    )) : <div className="rounded-[1.25rem] border border-white/10 bg-white/[0.03] p-6 text-sm text-white/55">No active tracks. Add one from the form to start a timed playlist.</div>}
                  </div>
                ) : null}
                ...
                {activeView === "expired-tracks" ? (
                  <div className="grid gap-4">
                    {liveExpiredTracks.length ? liveExpiredTracks.map((track) => (
                      <article key={track.id} className="rounded-[1.25rem] border border-white/10 bg-white/[0.03] p-4">
                    ...
                    )) : <div className="rounded-[1.25rem] border border-white/10 bg-white/[0.03] p-6 text-sm text-white/55">Expired tracks will appear here after their end time passes.</div>}
                  </div>
                ) : null}
              </>
            )}
