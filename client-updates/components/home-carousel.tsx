    ...
    <section className="surface-card overflow-hidden p-0" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
      <div className="grid gap-0 lg:grid-cols-[1.1fr,0.9fr]">
        <div className="flex flex-col justify-between gap-8 p-6 sm:p-8 lg:p-10">
          <div>
            <p className="eyebrow mb-4">Carousel</p>
            <div className="max-w-2xl transition-opacity duration-300">
              <p className="text-xs font-semibold uppercase tracking-[0.28em]" style={{ color: "var(--text-soft)" }}>{activeSlide.kicker}</p>
              {activeSlide.title ? <h2 className="mt-4 text-3xl font-semibold sm:text-5xl" style={{ color: "var(--text)" }}>{activeSlide.title}</h2> : null}
