...
import { getDetailedReleaseByUserId, updatePaidDistributionRelease } from "@/lib/distribution-db";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const existingRelease = await getDetailedReleaseByUserId(session.sub, parsed.metadata.editReleaseId);
    if (!existingRelease) return NextResponse.json({ error: "Release not found." }, { status: 404 });
    ...
    const artworkUrl = artworkUpload instanceof File
      ...
      : parsed.metadata.existingArtworkUrl ?? existingRelease.artworkUrl;
    ...
    for (const track of parsed.metadata.tracks) {
      ...
      const audioUrl = audioUpload instanceof File
        ...
        : track.existingAudioUrl ?? existingRelease.tracks?.find((item) => item.trackNumber === track.trackNumber)?.audioUrl ?? "";
