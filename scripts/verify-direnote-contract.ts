...
const htmlRejected = await submitToDireNote({}, { fetchImpl: async () => new Response("<html><head><title>Access denied</title></head><body>client=contract-client pin=contract-pin</body></html>", { status: 403, headers: { "content-type": "text/html" } }) });
assert.equal(htmlRejected.success, false);
assert.equal(htmlRejected.httpStatus, 403);
assert.match(htmlRejected.error ?? "", /AUTH_OR_ACCESS/);
assert.match(htmlRejected.error ?? "", /HTTP 403/);
assert.doesNotMatch(htmlRejected.raw ?? "", /contract-(client|pin)/);
const receiptImportRejected = await submitToDireNote({}, { fetchImpl: async () => new Response("Validation Error Error! Failed to upload license receipt. OK", { status: 200, headers: { "content-type": "application/json" } }) });
assert.match(receiptImportRejected.error ?? "", /^DIRENOTE_LICENSE_RECEIPT_IMPORT_FAILED/);
assert.equal(receiptImportRejected.providerReason, "licenseReceiptImportFailed");
const unavailable = await submitToDireNote({}, { fetchImpl: async () => new Response("<html>Temporarily unavailable</html>", { status: 503, headers: { "content-type": "text/html" } }) });
assert.match(unavailable.error ?? "", /PROVIDER_UNAVAILABLE/);
const unresponsive = await submitToDireNote({}, { timeoutMs: 5, fetchImpl: async () => new Promise<Response>(() => undefined) });
assert.equal(unresponsive.success, false);
assert.match(unresponsive.error ?? "", /timed out after 5 milliseconds/);
