...
export function AdminReferralActions({ referralId, rewarded }: { referralId: number; rewarded: boolean }) {
  ...
  async function run(action: "reverse" | "invalidate" | "approve") {
    ...
    const response = await fetch(`/api/admin/referrals/${referralId}/${action}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ reason, note: reason, confirm: true }) });
  ...
  return <div><div className="flex flex-wrap gap-2">{!rewarded && <button disabled={busy} className="btn-primary" onClick={() => run("approve")}>Approve eligible recurring reward</button>}{rewarded ? <button disabled={busy} className="btn-outline pressable" style={{ color: "var(--danger)" }} onClick={() => run("reverse")}>Reverse reward</button> : <button disabled={busy} className="btn-outline pressable" style={{ color: "var(--danger)" }} onClick={() => run("invalidate")}>Mark invalid</button>}<a className="btn-outline pressable" href="/admin/fraud">Review fraud</a></div>{message && <p className="mt-3 text-sm" role="status" style={{ color: "var(--text-muted)" }}>{message}</p>}</div>;
}
