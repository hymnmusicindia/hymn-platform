...
function AdminStoreStatusEditor({ release }: { release: Release }) {
  ...
  return <div className="rounded-[1.4rem] border p-4" style={{ borderColor: "var(--border)", background: "var(--bg-soft)" }}>
    ...
    {loading ? <p className="mt-4 text-sm" style={{ color: "var(--text-muted)" }}>Loading store statusesâ€¦</p> : drafts.length ? <div className="mt-4 grid gap-3">{drafts.map((row) => <div key={row.platform} className="rounded-xl border p-4" style={{ borderColor: "var(--border)", background: "var(--card)" }}>
    ...
    <div className="mt-4 flex flex-wrap gap-2"><button type="button" disabled={!changed.length || saving} onClick={save} className="btn-primary pressable">{saving ? "Savingâ€¦" : "Save Store Statuses"}</button><button type="button" disabled={!changed.length || saving} onClick={() => { setDrafts(saved); setFeedback(null); }} className="btn-outline pressable">Reset Changes</button></div>{feedback ? <p className="mt-3 text-sm" style={{ color: feedback.includes("saved") ? "var(--success)" : "var(--danger)" }}>{feedback}</p> : null}
    <details className="mt-5 rounded-xl border p-4" style={{ borderColor: "var(--border)" }}><summary className="cursor-pointer font-semibold">Store Status History ({history.length})</summary><div className="mt-4 grid gap-2">{history.map((item) => <div key={item.id} className="summary-card"><span>{new Date(item.updatedAt || "").toLocaleString()} Â· {item.platform}<br /><small>{item.updatedByLabel || "Admin"}{item.reason ? ` Â· ${item.reason}` : ""}</small></span><span>{item.oldStatus || "Not set"} â†’ {item.status}</span></div>)}{!history.length ? <p className="text-sm" style={{ color: "var(--text-muted)" }}>No status changes recorded yet.</p> : null}</div></details>
  </div>;
}
...
function uniqueReadinessIssues<T extends { field: string; message: string }>(issues: T[] = []) {
  return Array.from(new Map(issues.map((issue) => [`${issue.field}:${issue.message}`, issue])).values());
}
...
function AccountStatusBadge({ status = "active" }: { status?: User["status"] }) {
  ...
  return <span className="inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[11px] font-semibold capitalize" style={{ borderColor: healthy ? "color-mix(in srgb, #22c55e 45%, var(--border))" : caution ? "color-mix(in srgb, #eab308 50%, var(--border))" : "color-mix(in srgb, var(--danger) 50%, var(--border))", color: healthy ? "#22c55e" : caution ? "#eab308" : "var(--danger)" }}><span aria-hidden>{healthy ? "âœ“" : caution ? "âš " : "Ã—"}</span>{String(status).replaceAll("_", " ")}</span>;
}
  ...
  return <article className="surface-list-item overflow-hidden p-0">
    ...
    <div className="p-5 pt-0">
      ...
      {editing ? <form className="mt-5 grid gap-4 rounded-2xl border p-4" style={{ borderColor: "var(--border)", background: "var(--bg-soft)" }} onSubmit={(event) => { event.preventDefault(); onSave(producer.id, event.currentTarget); }}>
        ...
        <button type="submit" disabled={pending} className="btn-primary pressable disabled:opacity-50">{pending ? "Saving profileâ€¦" : "Save Beat Store profile"}</button>
      </form> : null}
    </div>
  </article>;
}
  ...
  return (
    <SurfaceSection title="Payout requests" description="Approve, process, pay, or reject artist payout requests. Sensitive payout details are masked here.">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3 border-b pb-4" style={{ borderColor: "var(--border)" }}>
        <div><p className="font-semibold">USD/INR payout rate</p><p className="mt-1 text-sm" style={{ color: "var(--text-muted)" }}>{fx?.usdToInrRate ? `1 USD = ${fx.usdToInrRate.toFixed(4)} INR Â· $105 â‰ˆ ${formatMoney(fx.approximateMinimumInr ?? 0)}` : "No successful rate stored"}{fx?.rateUpdatedAt ? ` Â· updated ${new Date(fx.rateUpdatedAt).toLocaleString("en-IN")}` : ""}{fx?.rateStatus === "stale" ? " Â· STALE" : ""}</p></div>
        <button type="button" className="btn-outline pressable" disabled={isPending} onClick={refreshFx}>{isPending ? "Refreshingâ€¦" : "Refresh rate"}</button>
      </div>
            ...
            <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
              <div>
                ...
                <div className="mt-3 grid gap-2 text-sm sm:grid-cols-2 lg:grid-cols-4" style={{ color: "var(--text-muted)" }}>
                  <span>Requested: {formatMoney(request.requestedAmount)}{request.requestedAmountUsd !== null ? ` Â· $${request.requestedAmountUsd.toFixed(2)}` : ""}</span>
                  {request.usdToInrRate !== null ? <span>FX: 1 USD = {request.usdToInrRate.toFixed(4)} INR Â· {request.exchangeRateProvider}</span> : null}
...
function AdminPayoutReports() {
  ...
  return <SurfaceSection title="Monthly and quarterly reports" description="Generate database-backed Excel workbooks, review closed periods, and operate quarterly payout cycles without deleting ledger history.">
    ...
    <div className="mt-5 overflow-x-auto rounded-xl border" style={{ borderColor: "var(--border)" }}><table className="min-w-full text-left text-sm"><thead><tr>{["Period", "Status", "Gross", "Artist pool", "Held", "Paid", "Carry forward", "Action"].map(h => <th key={h} className="px-3 py-3">{h}</th>)}</tr></thead><tbody>{data.periods.filter(p => p.type === "quarterly").map(period => <tr key={period.id} className="border-t" style={{ borderColor: "var(--border)" }}><td className="px-3 py-3">Q{period.quarter} {period.year}</td><td className="px-3 py-3 capitalize">{period.status}</td><td className="px-3 py-3">{formatMoney(Number(period.totalGrossRevenue))}</td><td className="px-3 py-3">{formatMoney(Number(period.totalArtistPool))}</td><td className="px-3 py-3">{formatMoney(Number(period.totalHeldAmount))}</td><td className="px-3 py-3">{formatMoney(Number(period.totalPaidAmount))}</td><td className="px-3 py-3">{formatMoney(Number(period.totalCarryForward))}</td><td className="px-3 py-3">{period.status === "open" ? <button className="btn-outline px-3 py-2 text-xs" onClick={() => closePeriod(period)}>Close Quarter</button> : period.generatedReportUrl ? <a className="btn-outline px-3 py-2 text-xs" href={period.generatedReportUrl}>Download</a> : "â€”"}</td></tr>)}</tbody></table></div>
    <div className="mt-5 grid gap-3">{data.reports.map(report => <article key={report.id} className="surface-list-item flex flex-wrap items-center justify-between gap-3 p-4"><div><p className="font-semibold">{report.fileName}</p><p className="mt-1 text-xs" style={{ color: "var(--text-soft)" }}>{report.type} Â· {new Date(report.generatedAt).toLocaleString("en-IN")} Â· {report.status}</p></div><div className="flex gap-2"><a href={`/api/payout/reports/${report.id}/download`} className="btn-outline px-3 py-2 text-xs">Download</a><button className="btn-outline px-3 py-2 text-xs" onClick={() => run({ type: report.type, month: report.month, quarter: report.quarter, year: report.year })}>Regenerate</button></div></article>)}</div>
  </SurfaceSection>;
}
...
export function AdminControlCenter({
  ...
  return (
    <DashboardFrame
      ...
      subtitle={<span className="admin-identity"><span className="admin-identity-line"><span className="admin-online-dot" />{currentAdmin.name}<span className="admin-active-label">Active</span></span><span className="admin-identity-meta">{currentAdmin.email} Â· {todayLabel}</span></span>}
      navItems={[
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border px-4 py-3 text-sm" style={{ borderColor: "var(--border)", background: "var(--bg-soft)" }}><div><span style={{ color: "var(--text-muted)" }}>Signed in as </span><strong>{currentAdmin.name}</strong><span style={{ color: "var(--text-muted)" }}> Â· Admin role: </span><strong className="capitalize">{adminAccess.role.replace(/_/g, " ")}</strong></div><span className="status-pill">{adminAccess.permissions.length} permissions</span></div>
        ...
        <div className="grid gap-6 xl:grid-cols-[0.82fr,1.18fr]">
          ...
          <SurfaceSection title={activeTab === "delivery" ? "Delivery Details" : "Catalog Details"} description="Distribution state, identifiers, stores, links, and reporting for the selected release.">
            {selectedCatalogRelease ? <div className="grid gap-5">
              <div className="flex flex-wrap items-start justify-between gap-4"><div><h3 className="text-2xl font-semibold">{selectedCatalogRelease.releaseTitle || selectedCatalogRelease.trackName}</h3><p className="mt-2 text-sm" style={{ color: "var(--text-muted)" }}>{selectedCatalogRelease.artistName} Â· {selectedCatalogRelease.releaseType.toUpperCase()} Â· Release {selectedCatalogRelease.releaseDate || "â€”"}</p></div><StatusPill label={selectedCatalogRelease.status.replace(/_/g, " ")} active /></div>
      ...
      {activeTab === "distribution-queue" ? (
        <div className="grid gap-6 xl:grid-cols-[0.92fr,1.08fr]">
                ...
                return <button key={release.id} type="button" onClick={() => { setSelectedReleaseId(release.id); setReviewTab("overview"); }} className="surface-list-item pressable p-4 text-left" style={selectedRelease?.id === release.id ? { borderColor: "var(--accent)", background: "var(--accent-soft)" } : undefined}>
                  <div className="flex gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate font-semibold" style={{ color: "var(--text)" }}>{release.releaseTitle || release.trackName}</p><div className="mt-1 flex flex-wrap items-center gap-2"><p className="truncate text-sm" style={{ color: "var(--text-soft)" }}>{release.artistName} Â· {release.tracks?.length ?? 0} track{release.tracks?.length === 1 ? "" : "s"} Â· {release.releaseType.toUpperCase()}</p><AccountStatusBadge status={users.find((user) => user.id === release.userId)?.status} /></div></div><StatusPill label={release.status.replace(/_/g, " ")} active /></div>
                      <div className="mt-3 grid gap-1 text-xs sm:grid-cols-2" style={{ color: "var(--text-muted)" }}><span>Submitted {release.submittedAt ? new Date(release.submittedAt).toLocaleDateString("en-IN") : "Not submitted"}</span><span>Release {release.releaseDate ? new Date(release.releaseDate).toLocaleDateString("en-IN") : "â€”"}</span><span>Payment {release.paymentStatus === "paid" ? "Verified" : release.paymentStatus ?? "Pending"}</span><span style={{ color: assetReady ? "var(--success)" : "var(--danger)" }}>Distribution {assetReady ? "Ready to check" : "Issues found"}</span></div>
                    </div>
                  </div>
                </button>;
              })}
          ...
          <SurfaceSection title="Selected Release Review" description="Inspect metadata, files, rights, readiness, and the complete activity record.">
            {selectedRelease ? (
              <div className="grid gap-5">
                <div className="flex flex-wrap items-start justify-between gap-4"><div><h3 className="text-2xl font-semibold" style={{ color: "var(--text)" }}>{selectedRelease.releaseTitle || selectedRelease.trackName}</h3><div className="mt-2 flex flex-wrap items-center gap-2"><p className="text-sm" style={{ color: "var(--text-muted)" }}>{selectedRelease.releaseType.toUpperCase()} Â· {selectedRelease.tracks?.length ?? 0} track{selectedRelease.tracks?.length === 1 ? "" : "s"} Â· {selectedRelease.artistName}</p><AccountStatusBadge status={users.find((user) => user.id === selectedRelease.userId)?.status} /></div><p className="mt-2 text-xs" style={{ color: "var(--text-soft)" }}>Submitted {selectedRelease.submittedAt ? new Date(selectedRelease.submittedAt).toLocaleString("en-IN") : "â€”"} Â· Release {selectedRelease.releaseDate || "â€”"} Â· {selectedRelease.ownerEmail || "Account email unavailable"}</p></div><StatusPill label={selectedRelease.status.replace(/_/g, " ")} active /></div>
                ...
                {reviewTab === "direnote" ? <div className="rounded-[1.4rem] border p-4" style={{ borderColor: "var(--border)", background: "var(--bg-soft)" }}>
                  ...
                  {direNoteReadiness ? (
                    <div className="mt-4 grid gap-3">
                      {uniqueReadinessIssues(direNoteReadiness.issues || []).length ? (
                        <div className="rounded-2xl border p-4 text-sm" style={{ borderColor: "color-mix(in srgb, var(--danger) 42%, var(--border))", background: "color-mix(in srgb, var(--danger-soft) 68%, var(--card))" }}>
                          <p className="font-semibold" style={{ color: "var(--text)" }}>{uniqueReadinessIssues(direNoteReadiness.issues || []).length} blocker{uniqueReadinessIssues(direNoteReadiness.issues || []).length === 1 ? "" : "s"} before DireNote handoff</p>
                          <p className="mt-1 text-xs" style={{ color: "var(--text-muted)" }}>Fix these fields, then reopen readiness or retry approval.</p>
                        </div>
                      ) : null}
                      {uniqueReadinessIssues(direNoteReadiness.issues || []).map((issue) => <div key={`${issue.category}-${issue.field}-${issue.message}`} className="rounded-xl border p-3 text-sm" style={{ borderColor: "var(--danger)", background: "var(--danger-soft)" }}><p className="font-semibold">{issue.category} - {readinessIssueLabel(issue)}</p><p className="mt-1">{issue.message}</p><p className="mt-1 text-xs">Fix: {issue.fixSuggestion}</p></div>)}
                      {uniqueReadinessIssues(direNoteReadiness.warnings || []).map((issue) => <div key={`warning-${issue.field}-${issue.message}`} className="rounded-xl border p-3 text-sm" style={{ borderColor: "var(--border)" }}><p className="font-semibold">Warning - {issue.category}</p><p className="mt-1">{issue.message}</p></div>)}
                    </div>
                  ) : null}
                </div> : null}
        ...
        <div className="grid gap-6">
          <section className="surface-card admin-module-section fade-up p-5 sm:p-6">
            ...
            <form className="mt-5 flex flex-col gap-2 lg:flex-row" onSubmit={(event) => { event.preventDefault(); setPaymentSearch(paymentSearchDraft); }}>
              <label className="relative min-w-0 flex-1"><span className="sr-only">Search payments</span><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2" style={{ color: "var(--text-soft)" }} /><input className="field w-full pl-10" value={paymentSearchDraft} onChange={(event) => setPaymentSearchDraft(event.target.value)} placeholder="Name, email, logged-in ID, order, payment ID, releaseâ€¦" /></label>
              ...
              {visibleDistributionPayments.map((order) => {
                ...
                return (
                  <article key={order.id} className="surface-list-item p-4">
                    ...
                    <div className="mt-4 grid gap-3 border-t pt-4 text-xs sm:grid-cols-2" style={{ borderColor: "var(--border)", color: "var(--text-muted)" }}><div><span className="block text-[10px] uppercase tracking-[0.12em]" style={{ color: "var(--text-soft)" }}>Order / plan</span><strong className="mt-1 block font-semibold" style={{ color: "var(--text)" }}>#{order.id} Â· {order.plan.replace(/_/g, " ")}</strong></div><div><span className="block text-[10px] uppercase tracking-[0.12em]" style={{ color: "var(--text-soft)" }}>Amount</span><strong className="mt-1 block font-semibold" style={{ color: "var(--text)" }}>{formatMoney(order.amount)}</strong></div><div><span className="block text-[10px] uppercase tracking-[0.12em]" style={{ color: "var(--text-soft)" }}>Release</span><strong className="mt-1 block truncate font-semibold" style={{ color: "var(--text)" }}>{linkedRelease?.releaseTitle || "Not linked yet"}{order.releaseId ? ` Â· #${order.releaseId}` : ""}</strong></div><div><span className="block text-[10px] uppercase tracking-[0.12em]" style={{ color: "var(--text-soft)" }}>Created</span><strong className="mt-1 block font-semibold" style={{ color: "var(--text)" }}>{new Date(order.createdAt).toLocaleString("en-IN")}</strong></div></div>
                    <div className="mt-3 rounded-lg border px-3 py-2 font-mono text-[11px] break-all" style={{ borderColor: "var(--border)", color: "var(--text-muted)" }}>Gateway order: {order.razorpayOrderId || "Not assigned"}{order.razorpayPaymentId ? ` Â· Payment: ${order.razorpayPaymentId}` : ""}</div>
                  </article>
                );
              })}
            ...
            <div className="grid gap-4">
              {visibleBeatStorePayments.map((order) => {
                ...
                return <article key={order.id} className="surface-list-item p-4">
                  ...
                  <div className="mt-4 grid gap-3 border-t pt-4 text-xs sm:grid-cols-2" style={{ borderColor: "var(--border)", color: "var(--text-muted)" }}><div><span className="block text-[10px] uppercase tracking-[0.12em]" style={{ color: "var(--text-soft)" }}>Order</span><strong className="mt-1 block" style={{ color: "var(--text)" }}>#{order.id} Â· {order.items.length} item{order.items.length === 1 ? "" : "s"}</strong></div><div><span className="block text-[10px] uppercase tracking-[0.12em]" style={{ color: "var(--text-soft)" }}>Amount</span><strong className="mt-1 block" style={{ color: "var(--text)" }}>{formatMoney(order.amount)}</strong></div><div><span className="block text-[10px] uppercase tracking-[0.12em]" style={{ color: "var(--text-soft)" }}>Purchase</span><strong className="mt-1 block truncate" style={{ color: "var(--text)" }}>{order.items.map((item) => item.beatTitle || `Beat #${item.beatId}`).join(", ") || "No items"}</strong></div><div><span className="block text-[10px] uppercase tracking-[0.12em]" style={{ color: "var(--text-soft)" }}>Created</span><strong className="mt-1 block" style={{ color: "var(--text)" }}>{new Date(order.createdAt).toLocaleString("en-IN")}</strong></div></div>
                  <div className="mt-3 rounded-lg border px-3 py-2 font-mono text-[11px] break-all" style={{ borderColor: "var(--border)", color: "var(--text-muted)" }}>Gateway order: {order.razorpayOrderId || "Not assigned"}{order.razorpayPaymentId ? ` Â· Payment: ${order.razorpayPaymentId}` : ""}</div>
                </article>;
              })}
              ...
              <article key={ticket.id} className="surface-list-item p-4">
                <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                  <div>
                    ...
                    <p className="mt-2 text-xs" style={{ color: "var(--text-soft)" }}>Category: {ticket.category ?? "general"}{ticket.relatedReleaseId ? ` Â· Release #${ticket.relatedReleaseId}` : ""}{ticket.relatedPurchaseId ? ` Â· Purchase #${ticket.relatedPurchaseId}` : ""}{ticket.relatedPayoutId ? ` Â· Payout #${ticket.relatedPayoutId}` : ""}</p>
                  </div>
      ...
      {activeTab === "producers" ? <div className="grid gap-6">
        ...
        <SurfaceSection title="Producer operations" description="Live creator accounts, storefront readiness, catalogue health, verified sales, and payout exposure.">
          <div className="grid gap-5 xl:grid-cols-2">
            ...
            {producerManagement.length === 0 ? <EmptyState copy="No producers found. Grant producer access from Admin Portal â†’ Users." /> : null}
          </div>
        </SurfaceSection>
      </div> : null}
            ...
            <div className="grid gap-4">
              {filteredMarketplaceBeats.map((beat) => (
                <article id={`admin-beat-${beat.id}`} key={beat.id} className="surface-list-item overflow-hidden p-0" style={initialBeatId === beat.id ? { outline: "2px solid var(--accent)", outlineOffset: "2px" } : undefined}>
                  <div className="grid md:grid-cols-[9rem,1fr]">{beat.artworkUrl ? <img src={beat.artworkUrl} alt={`${beat.title} cover`} className="aspect-square h-full min-h-36 w-full object-cover" /> : <div className="flex min-h-36 items-center justify-center border-b text-xs uppercase tracking-[0.18em] md:border-b-0 md:border-r" style={{ borderColor: "var(--border)", background: "var(--bg-soft)", color: "var(--text-soft)" }}>No artwork</div>}<div className="p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-lg font-semibold" style={{ color: "var(--text)" }}>{beat.title}</p><p className="mt-1 text-sm" style={{ color: "var(--text-muted)" }}>{beat.producerName || `Producer #${beat.producerId}`} Â· Submitted {new Date(beat.createdAt).toLocaleDateString("en-IN")}</p></div><StatusPill label={beat.status ?? (beat.enabled ? "PUBLISHED" : "PENDING_REVIEW")} active={beat.status === "PUBLISHED" || beat.status === "PENDING_REVIEW"} /></div><div className="mt-4 grid grid-cols-2 gap-x-5 gap-y-3 text-sm sm:grid-cols-4"><div><p className="text-xs" style={{ color: "var(--text-soft)" }}>Tempo</p><p className="mt-1 font-medium">{beat.bpm} BPM Â· {beat.keySignature || "Key missing"}</p></div><div><p className="text-xs" style={{ color: "var(--text-soft)" }}>Style</p><p className="mt-1 font-medium">{beat.genre} Â· {beat.mood}</p></div><div><p className="text-xs" style={{ color: "var(--text-soft)" }}>General</p><p className="mt-1 font-medium">{formatMoney(beat.generalPrice ?? beat.price)}</p></div><div><p className="text-xs" style={{ color: "var(--text-soft)" }}>Exclusive</p><p className="mt-1 font-medium">{formatMoney(beat.exclusivePrice ?? 0)}</p></div></div>{beat.reviewIssues?.reason ? <div className="mt-4 rounded-xl border px-3 py-2 text-sm" style={{ borderColor: "color-mix(in srgb, var(--danger) 45%, var(--border))", color: "var(--danger)" }}><strong>Corrections:</strong> {beat.reviewIssues.reason}</div> : null}<div className="mt-5 flex flex-wrap gap-2">{beat.previewUrl ? <a href={beat.previewUrl} target="_blank" rel="noreferrer" className="btn-outline pressable">Listen to preview</a> : <span className="btn-outline cursor-not-allowed opacity-50">Preview missing</span>}{beat.status !== "PUBLISHED" ? <button type="button" disabled={reviewingBeatId === beat.id || !hasPermission("users.manage")} onClick={() => reviewBeat(beat, "approved")} className="btn-primary pressable disabled:opacity-40">{reviewingBeatId === beat.id ? "Reviewingâ€¦" : "Approve and publish"}</button> : null}{beat.status !== "PUBLISHED" ? <button type="button" disabled={reviewingBeatId === beat.id || !hasPermission("users.manage")} onClick={() => reviewBeat(beat, "changes_requested")} className="btn-outline pressable disabled:opacity-40">Request corrections</button> : null}<button type="button" onClick={() => { setProducerBeatFilter(beat.producerId); setBeatStatusFilter("all"); }} className="btn-outline pressable">Producer catalogue</button></div></div></div>
                </article>
              ))}
          ...
          <section role="status" aria-label="Submitting to DireNote" className="w-full max-w-md rounded-[1.75rem] border p-8 shadow-2xl text-center flex flex-col items-center justify-center gap-5" style={{ borderColor: "var(--border)", background: "var(--card-strong)" }}>
            ...
            <div>
              ...
              <p className="mt-3 text-sm leading-relaxed" style={{ color: "var(--text-muted)" }}>
                ...
                <span className="font-semibold" style={{ color: "var(--text)" }}>This may take 1â€“2 minutes.</span> Please do not close or refresh this tab while we complete the distributor handshake.
              </p>
            </div>
          </section>
        </div>
