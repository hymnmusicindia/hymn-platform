...
import { adminReleaseStatusLabel } from "@/lib/release-status-presentation";
...
import type { AdminStoreStatus, ArtistProfile, Beat, DistributionOrder, Notification, Order, PartnershipLead, ProducerApplication, Release, SiteSettings, StoreStatus, StoreStatusHistoryEntry, SupportTicket, User } from "@/lib/types";
...
export function AdminControlCenter({
  ...
  const canSyncSelectedReleaseWithDireNote = Boolean(selectedRelease?.upcCode || selectedRelease?.direNoteStatus || selectedRelease?.tracks?.some(track => track.isrc));
  const [feedback, setFeedback] = useState<string | null>(null);
    ...
    >
        ...
        <div className="grid gap-6 xl:grid-cols-[0.82fr,1.18fr]">
          ...
          <SurfaceSection title={activeTab === "delivery" ? "Delivery Details" : "Catalog Details"} description="Distribution state, identifiers, stores, links, and reporting for the selected release.">
            {selectedCatalogRelease ? <div className="grid gap-5">
              <div className="flex flex-wrap items-start justify-between gap-4"><div><h3 className="text-2xl font-semibold">{selectedCatalogRelease.releaseTitle || selectedCatalogRelease.trackName}</h3><p className="mt-2 text-sm" style={{ color: "var(--text-muted)" }}>{selectedCatalogRelease.artistName} · {selectedCatalogRelease.releaseType.toUpperCase()} · Release {selectedCatalogRelease.releaseDate || "—"}</p></div><StatusPill label={adminReleaseStatusLabel(selectedCatalogRelease.status, selectedCatalogRelease.direNoteStatus)} active /></div>
              ...
              {selectedCatalogRelease.direNoteStatus && ["distribution", "identifiers"].includes(catalogTab) ? <ReleaseSubmissionHistory releaseId={selectedCatalogRelease.id} admin /> : null}
            ...
            {selectedRelease ? (
              <div className="grid gap-5">
                ...
                <div className="sticky bottom-3 z-10 grid gap-3 rounded-2xl border p-3 shadow-xl sm:grid-cols-2 xl:grid-cols-3" style={{ borderColor: "var(--border)", background: "var(--card-strong)" }}>
                  ...
                  <button type="button" disabled={isPending || !hasPermission("releases.read") || !canSyncSelectedReleaseWithDireNote} title={!hasPermission("releases.read") ? "Requires releases.read permission" : !canSyncSelectedReleaseWithDireNote ? "Available once DireNote has assigned a UPC or an ISRC exists for this release." : undefined} onClick={() => syncDireNoteRelease(selectedRelease.id)} className="btn-outline pressable disabled:opacity-45">Sync with DireNote</button>
      ...
      {(activeTab === "content" || activeTab === "settings") ? (
        <div className="grid gap-6">
          ...
          <AdminContentManager initialReleases={initialReleases} initialSiteSettings={initialSiteSettings} />
        </div>
      ) : null}
