...
export function GrowthAdminForm({ kind, campaignId, lead }: { kind: "campaign" | "spend" | "lead"; campaignId?: number; lead?: { id: number; status: string; notes: string; assignedTo: number | null } }) {
  ...
  return <form onSubmit={submit} className="grid gap-4 sm:grid-cols-2">
    {kind === "campaign" && <>{field("name", "Name")}{field("slug", "Unique campaign slug")}{field("channel", "Channel")}{field("source", "UTM source")}{field("medium", "UTM medium")}{field("content", "UTM content")}{field("budget", "Budget (INR)", "number", "3000")}<label className="grid gap-2 text-sm">Landing page<select className="field" name="landingPage" defaultValue="/first-release">{["/first-release", "/distribution", "/contact", "/partnership-program", "/"].map(path => <option key={path}>{path}</option>)}</select></label><label className="grid gap-2 text-sm">Status<select className="field" name="status">{["draft", "active", "paused", "completed"].map(status => <option key={status}>{status}</option>)}</select></label>{field("notes", "Hypothesis, creator, primary metric and decision")}</>}
