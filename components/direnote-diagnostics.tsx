...
export function DireNoteDiagnostics() {
  ...
  const load = () => fetch("/api/admin/direnote/diagnostics").then((r) => r.json()).then(setData).catch(() => setError("Could not load DireNote diagnostics."));
  useEffect(() => { void load(); }, []);
  async function test() { setBusy(true); setError(""); try { const response = await fetch("/api/admin/direnote/diagnostics", { method: "POST" }); const body = await response.json(); setData(body); if (!response.ok) setError(body.result?.error || JSON.stringify(body.result?.response) || "DireNote rejected the test payload."); } catch { setError("DireNote test request failed."); } finally { setBusy(false); } }
  return <section className="rounded-2xl border p-5" style={{ borderColor: "var(--border)", background: "var(--bg-soft)" }}>
    <h3 className="font-semibold">DireNote Diagnostics</h3><p className="mt-1 text-sm" style={{ color: "var(--text-muted)" }}>Credentials remain server-side and are never displayed.</p>
    ...
    {error ? <p className="mt-3 text-sm" style={{ color: "var(--danger)" }}>{error}</p> : null}<button type="button" onClick={test} disabled={busy || !data?.configReady} className="btn-primary pressable mt-4">{busy ? "Sending…" : "Send DireNote Test Payload"}</button>
  </section>;
}
// vercel trigger 8
