...
type Diagnostics = { endpointConfigured: boolean; pinConfigured: boolean; clientIdConfigured: boolean; configReady: boolean; lastSync?: { createdAt: string; success: boolean; responseJson: { checked?: number; eligible?: number } } | null; lastTest?: { success: boolean; httpStatus: number | null; response: unknown; createdAt: string } | null };
...
export function DireNoteDiagnostics() {
  ...
  return <section className="rounded-2xl border p-5" style={{ borderColor: "var(--border)", background: "var(--bg-soft)" }}>
    ...
    <div className="mt-4 text-sm"><strong>Last hourly sync:</strong> {data?.lastSync ? `${new Date(data.lastSync.createdAt).toLocaleString()} - ${data.lastSync.responseJson.checked ?? 0} checked - ${data.lastSync.success ? "Successful" : "Errors recorded"}` : "Not recorded"}</div>
