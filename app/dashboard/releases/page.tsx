...
export default async function ReleasePortalPage({ searchParams }: { searchParams?: Promise<{ releaseId?: string; panel?: string; tab?: string; view?: string }> }) {
  ...
  return (
    <main className="shell py-10 sm:py-12 lg:py-14">
      <ReleasePortal releases={releases} selectedReleaseId={Number.isInteger(selectedReleaseId) ? selectedReleaseId : null} initialPanel={params.panel ?? null} initialTab={params.tab ?? null} initialView={params.view ?? null} />
    </main>
  );
}
