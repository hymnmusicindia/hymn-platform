...
export default async function ReleaseChangeRequestsPage() {
  ...
  let requests: Awaited<ReturnType<typeof prisma.releaseChangeRequest.findMany>>;
  try { requests = await prisma.releaseChangeRequest.findMany({ orderBy: { submittedAt: "asc" }, take: 100, include: { release: { select: { title: true, artistName: true, status: true } }, events: { orderBy: { createdAt: "asc" } } } }); }
  catch {
