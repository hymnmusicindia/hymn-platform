...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json({ releaseId: release.id, trackCount: result.payload.tracks.length, ready: result.ready, issues: result.issues, warnings: result.warnings, checklist: result.checklist }, { headers: { "Cache-Control": "private, no-store" } });
}
// vercel trigger 9
