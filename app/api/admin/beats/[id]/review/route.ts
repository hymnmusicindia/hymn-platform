...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const updated = await prisma.beat.update({ where: { id }, data: { status: body.decision === "approved" ? "PUBLISHED" : "CHANGES_REQUESTED", enabled: body.decision === "approved", reviewIssues: body.decision === "approved" ? undefined : { reason: String(body.reason), issues: readiness.issues } } });
  await Promise.all([
