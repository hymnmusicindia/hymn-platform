...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (session.expiresAt <= new Date()) return NextResponse.json({ error: "This upload session expired. Start a new upload." }, { status: 410 });
  ...
  try {
    ...
    if (session.assetCategory === "TRACK_AUDIO_MASTER") await verifyAudioIntegrity(assembled.path, session.mimeType);
    if (session.assetCategory === "RELEASE_COVER_ART") {
      ...
      await verifyArtworkIntegrity(assembled.path);
    }
    const target = await finalRelativePath(session);
