...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (release.status !== "rejected" && release.status !== "changes_requested" && release.status !== "draft") return NextResponse.json({ error: "This release is not awaiting corrections." }, { status: 409 });
  const updated = await updateDetailedReleaseStatus(id, "under_review", "User corrected and resubmitted the release.");
  await Promise.all([
    logAuditEvent({ actorType: "user", actorId: user.session.sub, entityType: "release", entityId: id, action: "release.resubmitted", oldValue: { status: release.status }, newValue: { status: "under_review" } }),
