...
export async function POST(request: Request) {
  ...
  const release = await prisma.release.create({ data: { userId: user.user.id, title: String(body.title || "Untitled release"), artistName: user.user.name, genre: "", releaseDate: new Date(), status: "DRAFT", releaseType: "single", paymentStatus: "pending", draftCompletionPercent: 0, lastEditedAt: new Date(), missingFields, metadata: { draftCompletionPercent: 0, missingFields, lastEditedAt: new Date().toISOString() } } });
