...
export default async function DireNoteIntegrationPage() {
  ...
  const [lastSuccess, lastFailure, lastReleaseSync, lastRevenueSync, openDiscrepancies, unmatchedRevenue, lastSyncRun, lastSuccessfulSyncRun] = await Promise.all([
  ...
  const delayed = !lastSyncRun || Date.now() - lastSyncRun.startedAt.getTime() > 2 * 60 * 60 * 1000;
  const syncHealth = delayed ? "Delayed" : lastSyncRun.status === "completed" ? "Healthy" : "Failed";
  const metrics = [["Connection", config.isConfigured ? "Configured" : "Needs configuration"], ["DireNote Sync", syncHealth], ["Last sync run", date(lastSyncRun?.startedAt)], ["Last successful sync", date(lastSuccessfulSyncRun?.completedAt)], ["Candidates", String(lastSyncRun?.candidateCount ?? 0)], ["Processed / updated", `${lastSyncRun?.processedCount ?? 0} / ${lastSyncRun?.changedCount ?? 0}`], ["Identifier repairs", String(lastSyncRun?.identifierRepairCount ?? 0)], ["Sync errors", String(lastSyncRun?.errorCount ?? 0)], ["Last successful request", date(lastSuccess?.createdAt)], ["Last failed request", date(lastFailure?.createdAt)], ["Last release sync", date(lastReleaseSync?.direNoteLastSyncedAt)], ["Last revenue lookup", date(lastRevenueSync?.createdAt)], ["Open reconciliation issues", String(openDiscrepancies)], ["Unmatched revenue", String(unmatchedRevenue)]];
  return <main className="admin-panel-shell py-6 sm:py-8"><div className="mx-auto max-w-6xl"><section className="surface-card p-5 sm:p-7"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.16em]" style={{ color: "var(--text-soft)" }}>Integration monitor</p><h1 className="mt-2 text-3xl font-semibold">HYMN Distribution</h1><p className="mt-2 text-sm" style={{ color: "var(--text-muted)" }}>Operational status from HYMN&apos;s persisted distribution records. Credentials are never displayed.</p></div><Link href="/admin" className="btn-outline pressable">Back to Admin</Link></div><div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{metrics.map(([label, value]) => <div key={label} className="summary-card"><span>{label}</span><strong>{value}</strong></div>)}</div><div className="mt-7 flex flex-wrap gap-3"><Link className="btn-primary pressable" href="/admin?tab=distribution-queue">Open distribution queue</Link><Link className="btn-outline pressable" href="/admin?tab=revenue">Open revenue operations</Link></div></section><DireNoteReconciliationPanel /></div></main>;
}
